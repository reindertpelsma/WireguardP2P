...
In many setups you have a second router like a cellular modem or some home/tenant router connected to the ISP/building's CG-NAT infrastructure, meaning you have double NAT: your router and the ISP's router. You probbaly discovered that you can add a port forward rule in your router to your device but it has no effect as your ISP's firewall is blocking inbound connections, hence you ended up at this project. In that case it is still useful to add a port forward rule even if inbound connections from WAN are blocked, you do not want that your router is also blocking the inbound connection, plus in case the client is already on the internal network such as with Shared Router as explained below, then it can directly reach the remote device's wireguard.
