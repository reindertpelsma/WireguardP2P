...
4. The python script is just 270 lines and you do not need any other tool than wireguard, some linux commands, and python, far simpler than some commercial bloated P2P protocols like WebRTC. In almost all cases where WebRTC P2P works, this simple setup will work as well. The only extremely rare case where WebRTC P2P works and this project does not, is if the targeted remote device, not the client, is behind a NAT that does port randomization but is still restricted cone nat as the randomized ports are not per connection.
